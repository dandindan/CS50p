...
# Iterate over sorted metabolites
    for metabolite_value in sorted_metabolites:
        subset_df = df_sorted[df_sorted[metabolite_column] == metabolite_value]

        # Calculate average value for the subset
        avg_value = np.mean(subset_df['Value'])

        fig.add_trace(go.Violin(
            x=subset_df[metabolite_column],
            y=subset_df['Value'],
            marker=dict(size=3),
            # boxpoints=False,
            # notched=False,
            # Use metabolite value as the name for legend
            name=str(metabolite_value),
            # Text for hover labels
            text=f'{metabolite_value}<br>Average: {avg_value:.2f}',
            # hoverinfo='text',
            # boxmean=True,  # Display standard deviation line
            showlegend=False,  # Hide individual trace legend
        ))
        fig.add_annotation(
            x=metabolite_value,  # x-coordinate for annotation
            # y-coordinate for annotation (using logarithmic scale)
            y=np.log10(avg_value),
            # Text for annotation
            text=f'{metabolite_value}:<br> {avg_value:.1f}mM',
            showarrow=False,
            font=dict(size=10),  # Font settings for the annotation text
            xshift=0,  # Shift the annotation slightly to the right
            yshift=70,
            align='left',  # Align the annotation to the left of the box plot
        ...
        fig.update_layout(
            ...
            yaxis_type='log',
            # xaxis=dict(showgrid=True, dtick=1, gridwidth=0.05, gridcolor='rgba(255, 255, 255, 0.1)'),
            yaxis=dict(dtick=1, gridwidth=2,
                       gridcolor='rgba(125, 125, 125, .3)'),
            height=700,
            # width=800,
        )
